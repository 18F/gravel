...
const (
	...
	// ToS URL.
	ToSURL = "data:text/plain,Do%20what%20thou%20wilt"
)

// Options used to configure the WebFrontEnd. Most of these options are not exposed via the ACME standard, but they are
// left exposed as this is an integration test suite.
type WebFrontEndOpts struct {
	// Default: "/dir"
	...
	// Defaults: "/finalize-order/"
	OrderFinalizePath string
	...
	// Defaults: "/revoke-cert"
	RevokeCertPath string

	// Defaults: "/rollover-account-key"
	KeyRolloverPath string
	...
	// Defaults: "/intermediate-keys/"
	IntermediateKeyPath string

	// Defaults: "/cert-status-by-serial/"
	CertStatusBySerial string

	// How long do pending authorizations last before expiring?
	//   Defaults: 1 * time.Hour
	PendingAuthzExpire time.Duration

	// How many contacts is an account allowed to have?
	//   Defaults: 2
	MaxContactsPerAcct int

	// Defaults: 0
	BadNoncePercentage int

	// Defaults: 5
	DefaultNonceReject int
	...
	//   Defaults: 7
	UnusedRevocationReason uint
	//   Defaults: 10
	AACompromiseRevocationReason uint

	// AuthzReusePercentage defines an environment variable name used to provide a
	...
	//   Defaults: 0
	AuthzReusePercentage int
	// Defaults: 50
	DefaultAuthzReuse int
...
func NewDefaultWebFrontEndOpts() *WebFrontEndOpts {
	return &WebFrontEndOpts{
		...
		PendingAuthzExpire:           1 * time.Hour,
...
func New(db db.GravelStore, va *va.VerificationAuthority, ca *ca.CertificateAuthority, opts *WebFrontEndOpts) *WebFrontEnd {
	...
	wfe.log.Printf("configured to reject %d%% of good nonces", opts.BadNoncePercentage)
	wfe.log.Printf("configured to attempt authz reuse for each identifier %d%% of the time", opts.DefaultAuthzReuse)
...
type WebFrontEnd struct {
	...
	va                *va.VerificationAuthority
	ca                *ca.CertificateAuthority
	opts              *WebFrontEndOpts
}
	...
	defaultHandler := http.StripPrefix(pattern,
		&topHandler{
			wfe: wfeHandlerFunc(func(ctx context.Context, response http.ResponseWriter, request *http.Request) {
				...
				if request.Method == "POST" || pattern == wfe.opts.NoncePath {
				...
				if pattern != wfe.opts.DirectoryPath {
					directoryURL := wfe.relativeEndpoint(request, wfe.opts.DirectoryPath)
...
func (wfe *WebFrontEnd) HandleManagementFunc(
	mux *http.ServeMux,
	pattern string,
...
func (wfe *WebFrontEnd) sendError(prob *acme.ProblemDetails, response http.ResponseWriter) {
...
func (wfe *WebFrontEnd) handleCert(
	certGet certGetter,
	relPath string) func(
	ctx context.Context,
...
func (wfe *WebFrontEnd) handleKey(
	keyGet keyGetter,
	relPath string) func(
	ctx context.Context,
	response http.ResponseWriter,
...
func (wfe *WebFrontEnd) handleCertStatusBySerial(
	ctx context.Context,
	response http.ResponseWriter,
	request *http.Request) {

	serialStr := strings.TrimPrefix(request.URL.Path, wfe.opts.CertStatusBySerial)
	serial := big.NewInt(0)
...
func (wfe *WebFrontEnd) Handler() http.Handler {
	m := http.NewServeMux()
	// GET only handlers
	wfe.HandleFunc(m, wfe.opts.DirectoryPath, wfe.Directory, "GET")
	// Note for noncePath: "GET" also implies "HEAD"
	wfe.HandleFunc(m, wfe.opts.NoncePath, wfe.Nonce, "GET")

	// POST only handlers
	wfe.HandleFunc(m, wfe.opts.NewAccountPath, wfe.NewAccount, "POST")
	wfe.HandleFunc(m, wfe.opts.NewOrderPath, wfe.NewOrder, "POST")
	wfe.HandleFunc(m, wfe.opts.OrderFinalizePath, wfe.FinalizeOrder, "POST")
	wfe.HandleFunc(m, wfe.opts.AcctPath, wfe.UpdateAccount, "POST")
	wfe.HandleFunc(m, wfe.opts.KeyRolloverPath, wfe.KeyRollover, "POST")
	wfe.HandleFunc(m, wfe.opts.RevokeCertPath, wfe.RevokeCert, "POST")
	wfe.HandleFunc(m, wfe.opts.CertPath, wfe.Certificate, "POST")
	wfe.HandleFunc(m, wfe.opts.OrderPath, wfe.Order, "POST")
	wfe.HandleFunc(m, wfe.opts.AuthzPath, wfe.Authz, "POST")
	wfe.HandleFunc(m, wfe.opts.ChallengePath, wfe.Challenge, "POST")
...
func (wfe *WebFrontEnd) ManagementHandler() http.Handler {
	m := http.NewServeMux()
	// GET only handlers
	wfe.HandleManagementFunc(m, wfe.opts.RootCertPath, wfe.handleCert(wfe.ca.GetRootCert, wfe.opts.RootCertPath))
	wfe.HandleManagementFunc(m, wfe.opts.RootKeyPath, wfe.handleKey(wfe.ca.GetRootKey, wfe.opts.RootKeyPath))
	wfe.HandleManagementFunc(m, wfe.opts.IntermediateCertPath, wfe.handleCert(wfe.ca.GetIntermediateCert, wfe.opts.IntermediateCertPath))
	wfe.HandleManagementFunc(m, wfe.opts.IntermediateKeyPath, wfe.handleKey(wfe.ca.GetIntermediateKey, wfe.opts.IntermediateKeyPath))
	wfe.HandleManagementFunc(m, wfe.opts.CertStatusBySerial, wfe.handleCertStatusBySerial)
	return m
}

func (wfe *WebFrontEnd) Directory(
	ctx context.Context,
	response http.ResponseWriter,
	request *http.Request) {

	directoryEndpoints := map[string]string{
		"newNonce":   wfe.opts.NoncePath,
		"newAccount": wfe.opts.NewAccountPath,
		"newOrder":   wfe.opts.NewOrderPath,
		"revokeCert": wfe.opts.RevokeCertPath,
		"keyChange":  wfe.opts.KeyRolloverPath,
	}
...
func (wfe *WebFrontEnd) relativeDirectory(request *http.Request, directory map[string]string) ([]byte, error) {
...
func (wfe *WebFrontEnd) relativeEndpoint(request *http.Request, endpoint string) string {
	proto := "http"
	host := request.Host
...
func (wfe *WebFrontEnd) parseJWS(body string) (*jose.JSONWebSignature, error) {
...
func (wfe *WebFrontEnd) extractJWK(_ *http.Request, jws *jose.JSONWebSignature) (*jose.JSONWebKey, *acme.ProblemDetails) {
	header := jws.Signatures[0].Header
...
func (wfe *WebFrontEnd) lookupJWK(request *http.Request, jws *jose.JSONWebSignature) (*jose.JSONWebKey, *acme.ProblemDetails) {
	header := jws.Signatures[0].Header
	accountURL := header.KeyID
	prefix := wfe.relativeEndpoint(request, wfe.opts.AcctPath)
...
func (wfe *WebFrontEnd) validPOST(request *http.Request) *acme.ProblemDetails {
...
func (wfe *WebFrontEnd) validPOSTAsGET(postData *authenticatedPOST) (*core.Account, *acme.ProblemDetails) {
	if postData == nil {
...
// accounts.
func (wfe *WebFrontEnd) verifyPOST(
	request *http.Request,
...
func (wfe *WebFrontEnd) verifyJWSSignatureAndAlgorithm(
...
func (wfe *WebFrontEnd) extractJWSURL(
...
func (wfe *WebFrontEnd) verifyJWS(
	pubKey *jose.JSONWebKey,
	...
	if !wfe.nonce.validNonce(nonce) || nonceRoll < wfe.opts.BadNoncePercentage {
		return nil, acme.BadNonceProblem(fmt.Sprintf(
			"JWS has an invalid anti-replay nonce: %s", nonce))
	}

	expectedURL := url.URL{
		// NOTE(@cpu): ACME **REQUIRES** HTTPS and Gravel is hardcoded to offer the
		// API over HTTPS.
		Scheme: "https",
		Host:   request.Host,
...
func (wfe *WebFrontEnd) verifyContacts(acct acme.Account) *acme.ProblemDetails {
	...
	if len(contacts) > wfe.opts.MaxContactsPerAcct {
		return acme.MalformedProblem(fmt.Sprintf(
			"too many contacts provided: %d > %d", len(contacts), wfe.opts.MaxContactsPerAcct))
	}
...
func (wfe *WebFrontEnd) UpdateAccount(
	ctx context.Context,
	response http.ResponseWriter,
	request *http.Request) {
...
func (wfe *WebFrontEnd) verifyKeyRollover(
	innerPayload []byte,
...
func (wfe *WebFrontEnd) KeyRollover(
	ctx context.Context,
	response http.ResponseWriter,
	request *http.Request) {
	...
	if err != nil {
		if existingAccountError, ok := err.(*db.ExistingAccountError); ok {
			acctURL := wfe.relativeEndpoint(request, fmt.Sprintf("%s%s", wfe.opts.AcctPath, existingAccountError.MatchingAccount.ID))
...
func (wfe *WebFrontEnd) NewAccount(
	ctx context.Context,
	response http.ResponseWriter,
	request *http.Request) {
	...
	if existingAcct != nil {
		if existingAcct.Status == acme.StatusDeactivated {
			...
			acctURL := wfe.relativeEndpoint(request, fmt.Sprintf("%s%s", wfe.opts.AcctPath, existingAcct.ID))
			response.Header().Set("Location", acctURL)
	...
	acctURL := wfe.relativeEndpoint(request, fmt.Sprintf("%s%s", wfe.opts.AcctPath, newAcct.ID))
...
func (wfe *WebFrontEnd) verifyOrder(order *core.Order) *acme.ProblemDetails {
...
func (wfe *WebFrontEnd) makeAuthorizations(order *core.Order, request *http.Request) error {
	var auths []string
	...
	for _, name := range order.Identifiers {
		now := time.Now().UTC()
		expires := now.Add(wfe.opts.PendingAuthzExpire)
		...
		if authz == nil || rand.Intn(100) > wfe.authzReusePercent {
			...
			authz.URL = wfe.relativeEndpoint(request, fmt.Sprintf("%s%s", wfe.opts.AuthzPath, authz.ID))
		...
		authzURL := wfe.relativeEndpoint(request, fmt.Sprintf("%s%s", wfe.opts.AuthzPath, authz.ID))
...
func (wfe *WebFrontEnd) makeChallenge(
	chalType string,
	authz *core.Authorization,
	...
	chal := &core.Challenge{
		...
		Challenge: acme.Challenge{
			Type:   chalType,
			Token:  newToken(),
			URL:    wfe.relativeEndpoint(request, fmt.Sprintf("%s%s", wfe.opts.ChallengePath, id)),
			Status: acme.StatusPending,
		},
		Authz: authz,
	}
...
func (wfe *WebFrontEnd) makeChallenges(authz *core.Authorization, request *http.Request) error {
...
func (wfe *WebFrontEnd) NewOrder(
	...
	orderURL := wfe.relativeEndpoint(request, fmt.Sprintf("%s%s", wfe.opts.OrderPath, storedOrder.ID))
...
func (wfe *WebFrontEnd) orderForDisplay(
	order *core.Order,
	...
	result.Finalize = wfe.relativeEndpoint(request,
		fmt.Sprintf("%s%s", wfe.opts.OrderFinalizePath, order.ID))
	...
	if order.CertificateObject != nil {
		result.Certificate = wfe.relativeEndpoint(
			request,
			wfe.opts.CertPath+order.CertificateObject.ID)
	}
...
func (wfe *WebFrontEnd) Order(
	...
	orderID := strings.TrimPrefix(request.URL.Path, wfe.opts.OrderPath)
...
func (wfe *WebFrontEnd) FinalizeOrder(
	ctx context.Context,
	response http.ResponseWriter,
	request *http.Request) {
	...
	orderID := strings.TrimPrefix(request.URL.Path, wfe.opts.OrderFinalizePath)
	...
	orderURL := wfe.relativeEndpoint(request, fmt.Sprintf("%s%s", wfe.opts.OrderPath, existingOrder.ID))
	response.Header().Add("Location", orderURL)
	err = wfe.writeJSONResponse(response, http.StatusOK, orderReq)
	if err != nil {
		wfe.sendError(acme.InternalErrorProblem("Error marshaling order"), response)
...
func (wfe *WebFrontEnd) Authz(
	...
	authzID := strings.TrimPrefix(request.URL.Path, wfe.opts.AuthzPath)
...
func (wfe *WebFrontEnd) Challenge(
	ctx context.Context,
	response http.ResponseWriter,
	request *http.Request) {
	...
	chalID := strings.TrimPrefix(request.URL.Path, wfe.opts.ChallengePath)
...
func (wfe *WebFrontEnd) getAcctByKey(key crypto.PublicKey) (*core.Account, *acme.ProblemDetails) {
...
func (wfe *WebFrontEnd) validateChallengeUpdate(
...
func (wfe *WebFrontEnd) validateAuthzForChallenge(authz *core.Authorization) (*core.Order, *acme.ProblemDetails) {
...
func (wfe *WebFrontEnd) updateChallenge(
	...
	if wfe.opts.Strict && !bytes.Equal(postData.body, []byte("{}")) {
		wfe.sendError(
...
func (wfe *WebFrontEnd) Certificate(
	ctx context.Context,
	response http.ResponseWriter,
	request *http.Request) {
	postData, prob := wfe.verifyPOST(request, wfe.lookupJWK)
	...
	serialAlt := strings.TrimPrefix(request.URL.Path, wfe.opts.CertPath)
	...
	// Add links to alternate roots
	basePath := wfe.relativeEndpoint(request, fmt.Sprintf("%s%s", wfe.opts.CertPath, serial))
...
func (wfe *WebFrontEnd) writeJSONResponse(response http.ResponseWriter, status int, v interface{}) error {
...
func (wfe *WebFrontEnd) RevokeCert(
	ctx context.Context,
	response http.ResponseWriter,
	request *http.Request) {
...
func (wfe *WebFrontEnd) revokeCertByKeyID(
...
func (wfe *WebFrontEnd) revokeCertByJWK(
	jws *jose.JSONWebSignature,
	request *http.Request) *acme.ProblemDetails {
...
func (wfe *WebFrontEnd) processRevocation(
	jwsBody []byte,
	...
	if revokeCertReq.Reason != nil {
		...
		if r == wfe.opts.UnusedRevocationReason || r > wfe.opts.AACompromiseRevocationReason {
